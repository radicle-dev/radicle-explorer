<script lang="ts">
  ...
  import { retargetSeed, routeToPath } from "@app/lib/router";
  import { isExploreRouteId } from "@app/lib/routeId";
  ...
  async function applySeed(seed: BaseUrl) {
    ...
    if (navigateOnApply) {
      ...
      // Explore URLs pass through unchanged and rely on the updated
      // `selectedSeed` store, which `invalidateAll` makes their loads pick
      // up.
      await goto(retargetSeed(get(page).url, seed), { invalidateAll: true });
    } else {
      ...
      if (isExploreRouteId(get(page).route.id)) {
        await invalidateAll();
      }
    }
  }
