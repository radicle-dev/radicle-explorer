...
- When httpd cannot fill every field of `meta` — no canonical head, for example
  when delegates have not reached quorum, or an unreadable COB cache — it leaves
  out the whole project payload, as it did before this change. Older frontends
  require every field, so a partial `meta` would fail a whole listing for them.
  Newer frontends still get `cobs` and `defaultBranch`, and lose only the name
  and description.
...
- `payloads` stays a verbatim passthrough of delegate-signed data. Nothing in it
  ...
  scope for removal. The one exception is temporary: until 0.30.0, the project
  payload is left out when its `meta` would be incomplete.
- `payloads["xyz.radicle.project"].meta` is to be removed in httpd **0.30.0**.
  From then on the project payload is always passed through.
